...
<form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="bg-white border border-stone-100 shadow-sm p-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="settings[site_name]" value="{{ $settings['site_name'] ?? 'Cateura Accesorios' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">Descripción del sitio (meta)</label>
            <textarea name="settings[site_description]" rows="2" class="input-cateura border p-2 w-full">{{ $settings['site_description'] ?? '' }}</textarea>
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">Email de contacto</label>
            <input type="email" name="settings[contact_email]" value="{{ $settings['contact_email'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">Teléfono</label>
            <input type="text" name="settings[contact_phone]" value="{{ $settings['contact_phone'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">Dirección</label>
            <input type="text" name="settings[contact_address]" value="{{ $settings['contact_address'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
    </div>
    ...
    <div class="bg-white border border-stone-100 shadow-sm p-6 space-y-4">
        ...
        <div>
            ...
            <input type="url" name="settings[instagram_url]" value="{{ $settings['instagram_url'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">Facebook (URL)</label>
            <input type="url" name="settings[facebook_url]" value="{{ $settings['facebook_url'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">WhatsApp (número con código país, ej: 595981234567)</label>
            <input type="text" name="settings[whatsapp_number]" value="{{ $settings['whatsapp_number'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">Mensaje predeterminado WhatsApp</label>
            <input type="text" name="settings[whatsapp_message]" value="{{ $settings['whatsapp_message'] ?? 'Hola, me interesa un producto' }}" class="input-cateura border p-2 w-full">
        </div>
    </div>
    ...
    <div class="bg-white border border-stone-100 shadow-sm p-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="settings[google_analytics_id]" value="{{ $settings['google_analytics_id'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">Meta Pixel ID</label>
            <input type="text" name="settings[meta_pixel_id]" value="{{ $settings['meta_pixel_id'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">hCaptcha Site Key</label>
            <input type="text" name="settings[hcaptcha_site_key]" value="{{ $settings['hcaptcha_site_key'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-stone-600 mb-1">hCaptcha Secret Key</label>
            <input type="text" name="settings[hcaptcha_secret_key]" value="{{ $settings['hcaptcha_secret_key'] ?? '' }}" class="input-cateura border p-2 w-full">
        </div>
    </div>
