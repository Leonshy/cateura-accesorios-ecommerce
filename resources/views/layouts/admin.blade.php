    ...
    <aside class="bg-white border-r border-stone-200 flex-shrink-0 transition-all duration-300 hidden md:flex flex-col"
        ...
        <nav class="flex-1 overflow-y-auto px-2 py-4 space-y-1">
            ...
            <a href="{{ route('admin.reviews.index') }}" class="admin-nav-link {{ request()->routeIs('admin.reviews*') ? 'active' : '' }}">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.539 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.784.57-1.838-.196-1.539-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                <span x-show="sidebarOpen">Reseñas</span>
                @php $pendingReviews = \App\Models\ProductReview::where('is_approved', false)->count(); @endphp
                @if($pendingReviews > 0)
                <span x-show="sidebarOpen" class="ml-auto bg-copper-500 text-white text-[10px] font-bold rounded-full px-1.5 py-0.5">{{ $pendingReviews }}</span>
                @endif
            </a>
            @endif
            ...
            <a href="{{ route('admin.settings.shipping') }}" class="admin-nav-link {{ request()->routeIs('admin.settings.shipping') ? 'active' : '' }}">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m6 0a2 2 0 104 0"/></svg>
