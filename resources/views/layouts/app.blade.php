...
<head>
    ...
    {!! json_encode([
        ...
        '@type' => 'Organization',
        'name' => 'Cateura Accesorios',
        'url' => url('/'),
        'logo' => media_url(\App\Models\SiteSetting::get('site_logo'), asset('assets/brand/logo-horizontal.png')),
        'description' => 'Accesorios, piezas decorativas y prendas creadas por artesanas del Bañado Sur a partir de materiales reciclados.',
        'email' => \App\Models\SiteSetting::get('contact_email', 'asomujeresunidas22@gmail.com'),
        'telephone' => \App\Models\SiteSetting::get('contact_phone', '0981 877 315'),
        'address' => [
            '@type' => 'PostalAddress',
            'streetAddress' => 'Taller Productivo 43 Proyectadas c/ Capitán Figari, Barrio San Cayetano',
            'addressLocality' => 'Bañado Sur, Asunción',
            'addressCountry' => 'PY',
        ],
        'sameAs' => array_values(array_filter([
            ...
            \App\Models\SiteSetting::get('tiktok_url'),
        ], fn ($url) => is_complete_url($url))),
    ...
    @endif
    @if(\App\Services\HCaptchaVerifier::anyEnabled())
    <script src="https://js.hcaptcha.com/1/api.js" async defer></script>
    @endif
</head>
<body class="bg-white font-sans antialiased" x-data="{ mobileMenuOpen: false, cartOpen: false, searchOpen: false }">

@if(session('newsletter_pending_confirmation'))
<div x-data="{ show: true }" x-show="show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="show = false">
    <div class="absolute inset-0 bg-black/50" @click="show = false"></div>
    <div class="relative bg-white shadow-2xl w-full max-w-md p-8 text-center">
        <button @click="show = false" class="absolute top-4 right-4 text-stone-400 hover:text-stone-600 text-2xl leading-none" aria-label="Cerrar">×</button>
        <div class="w-16 h-16 bg-copper-100 rounded-full flex items-center justify-center mx-auto mb-5">
            <svg class="w-8 h-8 text-copper-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
        </div>
        <h2 class="font-display text-2xl text-stone-800 mb-3">¡Revisá tu correo! 📬</h2>
        <p class="text-stone-500 text-sm mb-6">Enviamos un enlace de confirmación a <strong class="text-stone-700">{{ session('newsletter_pending_confirmation') }}</strong>. Confirmá tu suscripción desde ese correo dentro de <strong class="text-stone-700">72 horas</strong>, o tu dirección se elimina automáticamente de la lista.</p>
        <button @click="show = false" class="btn-copper w-full">Entendido</button>
    </div>
</div>
...
@php
    $floatingWhatsappNumber = preg_replace('/\D/', '', (string) \App\Models\SiteSetting::get('whatsapp_number', '595981877315'));
@endphp
@if(strlen($floatingWhatsappNumber) >= 8)
<a href="https://wa.me/{{ $floatingWhatsappNumber }}?text={{ urlencode(\App\Models\SiteSetting::get('whatsapp_message', 'Hola, quiero consultar sobre los productos de Cateura Accesorios.')) }}"
