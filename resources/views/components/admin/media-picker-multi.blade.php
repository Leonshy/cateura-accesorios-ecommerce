...
<div x-data="mediaPickerMulti_{{ $pickerId }}()" x-init="init()">
    ...
    <div class="grid grid-cols-4 sm:grid-cols-6 gap-2 mb-3" x-show="images.length > 0">
        <template x-for="(url, index) in images" :key="url">
            <div class="relative aspect-square border border-stone-100 overflow-hidden group">
                <img :src="url" class="w-full h-full object-cover">
                <button type="button" @click="images.splice(index, 1)"
                        class="absolute top-1 right-1 bg-white/90 text-red-500 rounded-full w-5 h-5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-xs">✕</button>
            </div>
        </template>
    </div>
        ...
        <div class="relative bg-white shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col z-10">
            ...
            <div x-show="tab === 'library'" class="flex flex-col flex-1 overflow-hidden">
                ...
                <div class="p-4 border-t border-stone-100 flex justify-end gap-3 flex-shrink-0">
                    ...
                    <button type="button" @click="confirmSelection()" :disabled="!selectedUrls.length" class="px-4 py-2 bg-copper-600 text-white text-sm font-medium hover:bg-copper-700 disabled:opacity-40 transition-colors">
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-8" @dragover.prevent="uploadDragging = true" @dragleave.prevent="uploadDragging = false" @drop.prevent="handlePickerDrop($event)">
                    <label class="w-full cursor-pointer">
                        <div :class="uploadDragging ? 'border-copper-500 bg-copper-50' : 'border-stone-300 bg-stone-50'" class="border-2 border-dashed p-12 text-center transition-colors">
                            ...
                            <p class="text-sm text-stone-400">o hacé clic para seleccionar (podés elegir varios)</p>
                            <p class="text-xs text-stone-400 mt-2">Máximo 10 MB por archivo</p>
                        </div>
                        <input type="file" class="hidden" accept="{{ $accept }}" multiple @change="handlePickerUpload($event)">
                    </label>
            ...
            <div x-show="tab === 'url'" class="flex flex-col flex-1 overflow-hidden">
                <div class="flex-1 flex flex-col items-center justify-center p-8 gap-4">
                    ...
                    <button type="button" @click="addUrl()" :disabled="!manualUrl" class="px-6 py-2 bg-copper-600 text-white text-sm font-medium hover:bg-copper-700 disabled:opacity-40 transition-colors">Agregar esta URL</button>
                </div>
            </div>
...
<script>
function mediaPickerMulti_{{ $pickerId }}() {
    return {
        open: false,
        tab: 'library',
        images: {!! $initialJson !!},
        ...
        async deleteFile(file) {
            ...
            if (res.ok) {
                ...
                this.selectedUrls = this.selectedUrls.filter(u => u !== file.file_url);
            } else {
        ...
        toggleSelect(file) {
            const i = this.selectedUrls.indexOf(file.file_url);
            if (i === -1) this.selectedUrls.push(file.file_url);
            else this.selectedUrls.splice(i, 1);
        },

        confirmSelection() {
            this.selectedUrls.forEach(url => { if (!this.images.includes(url)) this.images.push(url); });
            this.selectedUrls = [];
            this.open = false;
        },

        addUrl() {
            if (this.manualUrl && !this.images.includes(this.manualUrl)) {
                this.images.push(this.manualUrl);
            }
            this.manualUrl = '';
            this.open = false;
        },
                ...
                await new Promise((resolve, reject) => {
                    xhr.onload = () => {
                        if (xhr.status === 201) {
                            ...
                            (data.uploaded ?? []).forEach(f => { if (!this.images.includes(f.file_url)) this.images.push(f.file_url); });
                            this.open = false;
                            resolve();
